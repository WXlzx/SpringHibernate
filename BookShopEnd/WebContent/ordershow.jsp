...
<%@page import="com.rj.bookshop.Entity.Order"%>
...
<div class="container">
	...
	<div ng-controller="cartController" class="container">
		<%
		Set<Order> orderset = (Set<Order>)request.getSession().getAttribute("orderset");
		
		if(orderset.size()>0){
		for(Order o :orderset){
			if(o.getOrderlineSet().size()>0){
		%>
		<table class="table" ng-show="cart.length">
			...
			<c:forEach var="order" items="${orderset }">
				...
				<tr ng-repeat="item in cart| filter:abc">
					<td >${orderline.book.book_id}</td>
					<td >${orderline.book.book_name}</td>
					<td>
						${orderline.count}
					</td>
					<td >${orderline.book.book_price}</td>
					<td > <fmt:formatNumber type="number" value="${orderline.price }" maxFractionDigits="2"/></td>
					<td><a href="deleteorder.do?orderlineid=${orderline.orderline_id}"><button type="button" class="btn btn-danger btn-sm">移除</button></a></td>
				</tr>
				</c:forEach>
	
				<tr>
					<td>总购物价</td>
					<td><fmt:formatNumber type="number" value="${price }" maxFractionDigits="2"/>元 </td>
					<td>总购买数量:</td>
					<td>${count}</td>
					<td colspan="2"><a href="deleteall.do?orderid=${order.order_id }"><button type="button"class="btn btn-danger btn-sm">清空购物车</button></a></td>
				</tr>
			</c:forEach>
			</tbody>	
		</table>
			...
			<div class="row" width="70%">
				<div class="col-xs-12">
					<div class="single-contact-field" >
						<%
						orderset = (Set<Order>)request.getSession().getAttribute("orderset");
						if(orderset.size()>0){
						for(Order o :orderset){
							if(o.getOrderlineSet().size()>0){
						%>
						<div class="contact-message-send" >
							<input type="submit" value="立即支付"  class="nv-btn"/>
						</div>
						<% break;}else{%>
						<div class="contact-message-send" >
							<a href="index.jsp"><button type="button" class="nv-btn">返回购买</button></a>
						</div>
						<%}}}else{
							%>
								<div class="contact-message-send" >
